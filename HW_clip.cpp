...
HW_clip(ImagePtr I1, int t1, int t2, ImagePtr I2)
{
	...
	// init lookup table
	int i, lut[MXGRAY];
	for (i = 0; i < t1 && i < MXGRAY; ++i) lut[i] = t1; //sets every output pixel to t1 if its input gray level is below t1.
	for (     ; i < t2 && i < MXGRAY; ++i) lut[i] =  i; //output pixel not change if its input gray level is in the range
	for (     ;           i < MXGRAY; ++i) lut[i] = t2;//sets every output pixel to t2 if its input gray level is above t2.
